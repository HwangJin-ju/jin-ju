...
public class BankApp {
	...
	// 인출할 계좌, 받을 계좌를 입력하면 송금하는 메소드
	public static void transferAmount() {
		while (true) {
			System.out.print("인출할 계좌>> ");
			String myNum = scn.next();
			Account user1 = searchAccountNo(myNum);

			if (user1 != null) {
				System.out.print("송금할 계좌>> ");
				String yourNum = scn.next();
				Account user2 = searchAccountNo(yourNum);
				{
					if (user2 != null) {
						System.out.println("입금할 금액>> ");
						int x = scn.nextInt();
						user1.setMoney(user1.getMoney() - x); 
						user2.setMoney(user2.getMoney() + x);
